...
Controlled by **Stores → Configuration → MageOS → SEO → Hreflang (Multistore) → Add Hreflang
Alternates to sitemap.xml**, and by **Enable Hreflang Tags** for the store view. They replace the
dedicated `/hreflang-sitemap.xml` earlier versions served, which is retired (see
[hreflang.md](hreflang.md#the-retired-hreflang-sitemapxml)).
